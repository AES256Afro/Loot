...
func _test_kingdom_map_and_living_world() -> void:
	...
	var resources_before_extraction: Dictionary = (state.get("resources", {}) as Dictionary).duplicate(true)
	var extracted := worlds.leave_dungeon_early(state)
	state = extracted.get("state", state)
	_assert(Vector2i(int(state.get("q", -1)), int(state.get("r", -1))) == Vector2i(10, 5) and String(state.get("active_view", "")) == "kingdom", "Early extraction returns to the exact entrance hex without forcing dungeon completion.")
	_assert((state.get("resources", {}) as Dictionary) == resources_before_extraction and String((state.get("quests", {}) as Dictionary).get("quest.reach.courtesy_drain_survey", "")) == "accepted", "Early extraction retains the accepted quest and cannot claim completion rewards.")
	entered = worlds.begin_dungeon(state, "site.reach.courtesy_drain")
	state = entered.get("state", state)
